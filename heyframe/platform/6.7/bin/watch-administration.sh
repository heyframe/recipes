...
export ADMIN_PORT
export APP_URL
export HEYFRAME_ADMIN_SKIP_SOURCEMAP_GENERATION
export DISABLE_DEVSERVER_OPEN
export DDEV_PRIMARY_URL

if [[ -e "${PROJECT_ROOT}/vendor/heyframe/platform" ]]; then
    ADMIN_ROOT="${ADMIN_ROOT:-"${PROJECT_ROOT}/vendor/heyframe/platform/src/Administration"}"
else
    ADMIN_ROOT="${ADMIN_ROOT:-"${PROJECT_ROOT}/vendor/heyframe/administration"}"
fi
...
# Dump entity schema
if [[ -z "${HEYFRAME_SKIP_ENTITY_SCHEMA_DUMP:-""}" ]] && [[ -f "${ADMIN_ROOT}"/Resources/app/administration/scripts/entitySchemaConverter/entity-schema-converter.ts ]]; then
